...
const BrReport = () => {
  ...
  const handleBrReport = () => {
    fetch(`http://67.21.32.75:5090/baReport/${brDate}`)
      .then((res) => res.json())
      .then((data) => setStatus(data));
  };
  const handleTerritoryReport = () => {
    fetch(`http://67.21.32.75:5090/territoryReport/${territoryDate}`)
      .then((res) => res.json())
      .then((data) => setTerritoryStatus(data));
  };
  const handleAreaReport = () => {
    fetch(`http://67.21.32.75:5090/areaReport/${areaDate}`)
      .then((res) => res.json())
      .then((data) => setAreaStatus(data));
  };
  const handleRegionReport = () => {
    fetch(`http://67.21.32.75:5090/regionReport/${regionDate}`)
      .then((res) => res.json())
